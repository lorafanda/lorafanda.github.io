...
const mobaState = {
  ...
  brainSurfaceMode: 'pial',  // 'pial' | 'inflated' — controlled by toggleSurfaceMode()
  t1OverlayOn:      false,   // T1 volume overlay (off by default to save bandwidth)
  t1Opacity:        0.30,    // 0..1 opacity for the loaded T1 volume
...
// ─────────────────────────────────────────────────────────────────────────────
// Brain-surface (pial vs inflated) + T1 volume overlay helpers
// ─────────────────────────────────────────────────────────────────────────────
const _MESH_RGBA_BONE = [205, 198, 190, 30];

// Mode-aware mesh URLs. The inflated variant is generated by
// scripts/build_fsaverage_meshes.py and committed to the meshes/ dir.
function _meshUrlsForMode(mode) {
  const suffix = mode === 'inflated' ? '.inflated' : '';
  return [
    `${FSAV_MESH_BASE}/fsaverage_lh${suffix}.mz3`,
    `${FSAV_MESH_BASE}/fsaverage_rh${suffix}.mz3`,
  ];
}

// Load fsaverage meshes for the given surface mode. Falls back to .gii
// if .mz3 fails. Returns true on success.
async function loadBrainSurfaceMeshes(nv, mode) {
  const urls = _meshUrlsForMode(mode);
  try {
    const spec = urls.map(u => ({ url: u, rgba255: _MESH_RGBA_BONE, visible: true }));
    ...
    // .gii fallback
    try {
      const spec = urls.map(u => ({ url: u.replace('.mz3', '.gii'), rgba255: _MESH_RGBA_BONE }));
      await nv.loadMeshes(spec);
      mobaState._meshSpec = spec;
      mobaState.meshLoaded = true;
      return true;
    } catch (e2) {
      console.warn(`[MOBA] Could not load ${mode} meshes:`, e2.message);
      return false;
    }
  }
}

// User-facing toggle for the surface mode (Pial vs Inflated).
async function toggleSurfaceMode() {
  const nv = mobaState.nv;
  if (!nv) return;
  const newMode = mobaState.brainSurfaceMode === 'pial' ? 'inflated' : 'pial';
  // Drop existing brain meshes; loadBrainSurfaceMeshes adds the new ones.
  for (const m of (nv.meshes || []).slice()) {
    if (typeof m.name === 'string' && /fsaverage/i.test(m.name)) {
      try { nv.removeMesh(m); } catch (e) {}
    }
  }
  const ok = await loadBrainSurfaceMeshes(nv, newMode);
  if (!ok) {
    setStatus(`Inflated meshes not available. Run scripts/build_fsaverage_meshes.py and commit the new .mz3 files.`);
    return;
  }
  mobaState.brainSurfaceMode = newMode;
  // Match the existing pial-load opacity treatment.
  ...
  document.getElementById('surfaceBtn').textContent =
...
async function toggleT1Overlay() {
  const nv = mobaState.nv;
  if (!nv) return;
  const btn = document.getElementById('t1Btn');
  const slider = document.getElementById('t1OpacityWrap');
  if (mobaState.t1OverlayOn) {
    // Off: drop the volume.
    try {
      for (const v of (nv.volumes || []).slice()) {
        if (typeof nv.removeVolume === 'function') nv.removeVolume(v);
      }
    } catch (e) { console.warn('[MOBA] removeVolume:', e); }
    mobaState.t1OverlayOn = false;
    btn.classList.remove('active');
    if (slider) slider.style.display = 'none';
    if (typeof nv.drawScene === 'function') nv.drawScene();
    return;
  }
  // On: load the volume. URL points at the analysis repo's mesh dir.
  const url = `${FSAV_MESH_BASE}/fsaverage_T1.nii.gz`;
  try {
    await nv.loadVolumes([{
      ...
      cal_max: 255,
      colormap: 'gray',
      opacity: mobaState.t1Opacity,
    }]);
    mobaState.t1OverlayOn = true;
    btn.classList.add('active');
    if (slider) slider.style.display = 'inline-flex';
    if (typeof nv.drawScene === 'function') nv.drawScene();
  } catch (e) {
    console.warn('[MOBA] T1 load failed:', e);
    setStatus('T1 volume not available. Run scripts/build_fsaverage_meshes.py and commit fsaverage_T1.nii.gz.');
  }
}

// Slider handler — updates volume opacity in place without reloading.
function setT1Opacity(pct) {
  const v = Math.max(0, Math.min(100, +pct || 0)) / 100;
  mobaState.t1Opacity = v;
  const nv = mobaState.nv;
  if (!nv || !mobaState.t1OverlayOn) return;
  for (const vol of (nv.volumes || [])) {
    if ('opacity' in vol)      vol.opacity = v;
    if ('alphaThreshold' in vol) vol.alphaThreshold = v < 0.05;
  }
  if (typeof nv.drawScene === 'function') nv.drawScene();
}
...
async function initBrain() {
  ...
  // Mode-aware: 'pial' loads fsaverage_lh.mz3 / fsaverage_rh.mz3;
  //             'inflated' loads fsaverage_lh.inflated.mz3 / rh equivalent.
  // The inflated meshes need to exist in the repo — run
  // scripts/build_fsaverage_meshes.py and commit the new .mz3 files.
  mobaState._meshSpec = null;
  const meshLoaded = await loadBrainSurfaceMeshes(nv, mobaState.brainSurfaceMode);
  if (!meshLoaded) {
    setStatus('Mesh load failed. Run scripts/build_fsaverage_meshes.py once to (re)generate the .mz3 files.');
    return;
  }
